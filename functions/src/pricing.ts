...
const inr = (monthly: number, annual: number): CountryPricing => ({
  currency: "INR",
  symbol: "₹",
  decimals: 2,
  dialCode: "+91",
...
const eur = (dialCode: string): CountryPricing => ({
  currency: "EUR",
  symbol: "€",
  decimals: 2,
  dialCode,
...
export function pricingFor(catalog: PlanCatalog, countryCode: string): CountryPricing | undefined {
 ...
 */
export function countryFromPhone(catalog: PlanCatalog, phoneNumber: string): string | undefined {
...
export function isValidPricing(value: unknown): value is CountryPricing {
  const row = value as CountryPricing;
  return (
    !!row &&
