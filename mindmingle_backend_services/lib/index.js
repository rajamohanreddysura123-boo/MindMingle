 ...
 */
function stripLocation(user, callerLat, callerLng) {
    const { latitude, longitude, ...rest } = user;
    const canMeasure = callerLat !== null && callerLng !== null &&
        typeof latitude === "number" && typeof longitude === "number";
    if (!canMeasure)
        return rest;
    const km = haversineKm(callerLat, callerLng, latitude, longitude);
    return {
        ...rest,
...
exports.filterDiscoverProfiles = (0, https_1.onCall)(async (request) => {
    ...
    const usesPremiumFilters = maxDistanceKm !== null ||
        countries.length > 0 ||
        districts.length > 0 ||
        languages.length > 0 ||
        interests.length > 0 ||
    ...
    const isShowable = (user) => {
        ...
        if (user.isDeletionRequested)
            return false;
        if (user.isDeactivated && Date.now() < Number(user.reactivateAt ?? 0))
            return false;
        return true;
    };
    /** Every filter the caller actually set. */
    const matchesExactly = (user) => {
        ...
        if (occupations.length > 0 && !occupations.includes(user.occupation))
            return false;
        if (countries.length > 0 && !countries.includes(countryCodeOf(user)))
            return false;
    ...
    while (exact.length < deckSize && scanned < SCAN_CAP) {
        ...
        for (const doc of page.docs) {
            ...
            if (matchesExactly(user)) {
                exact.push(user);
                if (exact.length >= deckSize)
                    break;
            }
            else if (matchesLoosely(user)) {
                if (related.length < deckSize)
                    related.push(user);
            }
